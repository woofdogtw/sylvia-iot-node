...
function newWrongOpts(engine) {
  return function () {
    assert.throws(() => new engine.Connection(null));
    assert.throws(() => new engine.Connection({ uri: 1 }));
    assert.throws(() => new engine.Connection({ uri: ':://' }));
    assert.throws(() => new engine.Connection({ uri: 'amq://localhost' }));
    assert.throws(() => new engine.Connection({ connectTimeoutMillis: 0.1 }));
    assert.throws(() => new engine.Connection({ reconnectMillis: 0.1 }));
    assert.throws(() => new engine.Connection({ insecure: 0 }));

    if (engine === gmq.mqtt) {
      assert.throws(() => new engine.Connection({ clientId: '012345678901234567890123' }));
      assert.throws(() => new engine.Connection({ cleanSession: 0 }));
    }
  };
}
