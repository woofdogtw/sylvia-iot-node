...
class TestRecvMsgHandler {
  ...
  onMessage(queue, msg) {
    if (this.useNack) {
      queue.nack(msg, (err) => {
        if (err) {
          this.nackErrors.push(err);
        } else {
          this.nackMessages.push(msg.payload);
        }
      });
    } else {
      queue.ack(msg, (err) => {
        if (err) {
          this.ackErrors.push(err);
        } else {
          this.recvMessages.push(msg.payload);
        }
      });
    }
  }
...
function sendError(engine) {
  return function (done) {
    ...
    assert.doesNotThrow(() => queue.setMsgHandler((_queue, _msg) => {}));
    assert.throws(() => queue.sendMsg('payload'));
    assert.throws(() => queue.sendMsg(Buffer.from(''), {}));
...
function dataAckNackWrong(engine) {
  return function () {
    ...
    assert.throws(() => queue.ack(null, () => {}));
    assert.throws(() => queue.ack({}, {}));
    assert.throws(() => queue.nack(null, () => {}));
    assert.throws(() => queue.nack({}, {}));
  };
}
...
function createMsgRsc(engine, opts, receiverCount, callback) {
  ...
  process.nextTick(() => callback(null, retHandlers));
}
