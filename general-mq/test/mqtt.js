...
const gmq = require('..');
const callbackGmq = require('../lib/callback');
...
describe('callback/mqtt', function () {
  ...
  describe('MqttConnection', function () {
    ...
    it('addPacketHandler() with wrong parameters', callbackAddPacketHandlerWrong);
    it('removePacketHandler() with wrong parameters', callbackRemovePacketHandlerWrong);
...
function addPacketHandlerWrong() {
  const conn = new gmq.mqtt.Connection();
  assert.ok(conn);
  assert.throws(() => conn.addPacketHandler('A@'));
  assert.throws(() => conn.addPacketHandler('name', 'name1'));
  assert.throws(() => conn.addPacketHandler('name', 'name', 0));
  assert.throws(() => conn.addPacketHandler('name', 'name', false, {}));
}

function removePacketHandlerWrong() {
  const conn = new gmq.mqtt.Connection();
  assert.ok(conn);
  assert.throws(() => conn.removePacketHandler('A@'));
}

function callbackAddPacketHandlerWrong() {
  const conn = new callbackGmq.mqtt.Connection();
  assert.ok(conn);
  assert.throws(() => conn.addPacketHandler('A@'));
  assert.throws(() => conn.addPacketHandler('name', 'name1'));
  assert.throws(() => conn.addPacketHandler('name', 'name', 0));
...
function callbackRemovePacketHandlerWrong() {
  const conn = new callbackGmq.mqtt.Connection();
  assert.ok(conn);
  assert.throws(() => conn.removePacketHandler('A@'));
}
