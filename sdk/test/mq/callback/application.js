...
const {
  ...
  OnAppDlDataResp,
  OnAppDlDataResult,
} = require('../../../mq/callback/application');
    ...
    async.waterfall(
      [
        ...
        function (cb) {
          ...
          mgr.sendDlData(data1, (err) => cb(err || null));
        },
        function (cb) {
          data2 = {
            correlationId: '2',
          ...
          mgr.sendDlData(data2, (err) => cb(err || null));
        },
        function (cb) {
          (function waitFn(retry) {
            if (retry < 0) {
              return cb(Error(`only receive ${recvDldata.length}/2 data`));
            } else if (recvDldata.length === 2) {
              return cb(null);
            }
