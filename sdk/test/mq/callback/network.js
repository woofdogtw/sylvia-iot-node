...
const { Events, Status } = require('general-mq').constants;
...
const {
  ...
  NetCtrlMsg,
  OnNetDlData,
} = require('../../../mq/callback/network');
const mqSdkLib = require('../../../mq/callback/lib');
...
function newWrongOpts(engine) {
  return function (done) {
    ...
    mqSdkLib.removeConnection(connPool, new URL('tcp://localhost'), 0, (err) => done(err || null));
  };
}
    ...
    async.waterfall(
      [
        ...
        function (cb) {
          ...
          mgr.sendUlData(data1, (err) => cb(err || null));
        },
        function (cb) {
          data2 = {
          ...
          mgr.sendUlData(data2, (err) => cb(err || null));
        },
        function (cb) {
          (function waitFn(retry) {
            if (retry < 0) {
              return cb(Error(`only receive ${recvUldata.length}/2 data`));
            } else if (recvUldata.length === 2) {
              return cb(null);
            }
            setTimeout(() => waitFn(retry - 1), 10);
          })(100);
        },
        function (cb) {
          for (let i = 0; i < 2; i++) {
        ...
        function (cb) {
          ...
          (function waitFn(retry) {
            if (retry < 0) {
              ...
              return void setTimeout(() => waitFn(retry - 1), 10);
            }
            ...
            for (let i = 0; i < expectCount; i++) {
              ...
              if (dataId === '1') {
                ...
                return void cb(Error(`unexpected dataId ${dataId}`));
              }
            }
            cb(null);
          })(100);
    ...
    async.waterfall(
      [
        ...
        function (cb) {
          ...
          mgr.sendDlDataResult(data1, (err) => cb(err || null));
        },
        function (cb) {
          data2 = {
            dataId: '2',
          ...
          mgr.sendDlDataResult(data2, (err) => cb(err || null));
        },
        function (cb) {
          (function waitFn(retry) {
            if (retry < 0) {
              return cb(Error(`only receive ${recvDldataResult.length}/2 data`));
            } else if (recvDldataResult.length === 2) {
              return cb(null);
            }
            setTimeout(() => waitFn(retry - 1), 10);
          })(100);
