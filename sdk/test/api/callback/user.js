...
const { Client, ClientOptions } = require('../../../api/callback/http');
const userapi = require('../../../api/callback/user');
...
function testGetErr(done) {
  ...
  assert.throws(() => userapi.get({}));
  assert.throws(() => userapi.get(client, {}));

  userapi.get(client, (err, data) =>
    done(err ? null : Error(`should error, data: ${JSON.stringify(data)}`))
  );
}
...
function testUpdate(done) {
  ...
  userapi.update(client, { name: 'test' }, (err) => done(err || null));
}
...
function testUpdateErr(done) {
  ...
  assert.throws(() => userapi.update({}));
  assert.throws(() => userapi.update(client, null));
  assert.throws(() => userapi.update(client, {}, {}));
