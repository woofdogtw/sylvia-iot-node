...
function testReqErr(done) {
  ...
  assert.throws(() => client.request('GET', ''));
  assert.throws(() => client.request('GET', '/api', {}));
  assert.throws(() => client.request('GET', '/api', {}, {}));
  assert.throws(() => client.request('GET', '/api', null, () => {}));
  ...
  async.waterfall(
    [
      function (cb) {
        ...
        const client = new Client(opts);
        client.request('GET', '/api', (err) => cb(err ? null : 'wrong authBase should error'));
      },
      function (cb) {
        const opts = {
          authBase: AUTH_BASE,
        ...
        const client = new Client(opts);
        client.request('GET', '/api', (err) => cb(err ? null : 'wrong coremgrBase should error'));
      },
    ],
    done
  );
